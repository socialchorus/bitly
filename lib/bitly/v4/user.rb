...
module Bitly
  module V4
    ...
    #
    #     o = Bitly::V4::OAuth.new(consumer_token, consumer_secret)
    #     o.authorize_url(redirect_url)
    #     #=> "https://api-ssl.bitly.com/v4/oauth/authorize?client_id=#{consumer_token}&type=web_server&redirect_uri=http%3A%2F%2Ftest.local%2Fbitly%2Fauth"
    ...
    #
    #    u=Bitly::V4::User.new(o.access_token)
    class User
      include HTTParty
      base_uri 'https://api-ssl.bit.ly/v4/'
      ...
      def referrers(opts = {})
        if @referrers.nil? || opts.delete(:force)
          @referrers = get_method(:referrers, Bitly::V4::Referrer, opts)
        end
        @referrers
      end
      ...
      def countries(opts = {})
        if @countries.nil? || opts.delete(:force)
          @countries = get_method(:countries, Bitly::V4::Country, opts)
        end
        @countries
      end
      ...
      def realtime_links(opts = {})
        if @realtime_links.nil? || opts.delete(:force)
          ...
          if result['status_code'] == 200
            @realtime_links = result['data']['realtime_links'].map { |rs| Bitly::V4::RealtimeLink.new(rs) }
          else
      ...
      def get_method(method, klass, opts)
        opts.merge!(access_token: @access_token.token)
        result = self.class.get("/user/#{method}", query: opts)
        if result['status_code'] == 200
      ...
      def get_clicks(opts = {})
        if @clicks.nil? || opts.delete(:force)
          ...
          if result['status_code'] == 200
            @clicks = result['data']['clicks'].map { |rs| Bitly::V4::Day.new(rs) }
